use crate::{
    db,
    model::{self, book_class::BookClass},
};
...
pub(crate) async fn list(mut db: Connection<db::Conn>, page_no: i32, page_size: i32) -> Resp<PageWrap<Vec<BookClass>>> {
    let mut tab = model::book_class::Table::new(&mut db);
...
pub(crate) async fn add(mut db: Connection<db::Conn>, b: BookClass) -> Resp<u64> {
    let mut tab = model::book_class::Table::new(&mut db);
...
pub(crate) async fn find_by_id(mut db: Connection<db::Conn>, id: i32) -> Resp<Option<BookClass>> {
    let mut tab = model::book_class::Table::new(&mut db);
...
pub(crate) async fn edit(mut db: Connection<db::Conn>, b: BookClass) -> Resp<u64> {
    let mut tab = model::book_class::Table::new(&mut db);
